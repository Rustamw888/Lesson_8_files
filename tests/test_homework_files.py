...
def test_unpacked_files():
    ...
    fantasy_zip.extractall('../unpacked_files/')
    fantasy_zip.close()


def test_csv():
    with open('unpacked_files/csvSample.csv') as f:
        reader = csv.reader(f)
        headers = next(reader)
...
def test_pdf():
    reader = PdfReader('unpacked_files/pdfSample.pdf')
    page = reader.pages[2]
...
def test_txt():
    reader = open('unpacked_files/txtSample.txt')
    assert reader.readline() == 'Quod equidem non reprehendo;\n'


def test_xlsx():
    workbook = load_workbook('unpacked_files/xlsxSample.xlsx')
